 ...
 *
 */
@Command(name = "sha512", description = "Compute the SHA512 hash of given file/file pattern(s)")
public class SHA512 extends AbstractMultiFileTool {
	...
	public static void main(String[] args) {
		SHA512 sha512 = SingleCommand.singleCommand(SHA512.class).parse(args);
		
		if(sha512.helpOption.showHelpIfRequested()) {
			return;
		}
		
		if(AssertUtils.isEmpty(sha512.arguments)) {
			Help.help(sha512.helpOption.commandMetadata);
			return;
		}
		
		sha512.execute(sha512.arguments.toArray(StringUtils.EMPTY_STRING_LIST));
	}
