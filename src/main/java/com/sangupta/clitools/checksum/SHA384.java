 ...
 *
 */
@Command(name = "sha384", description = "Compute the SHA384 hash of given file/file pattern(s)")
public class SHA384 extends AbstractMultiFileTool {
	...
	public static void main(String[] args) {
		SHA384 sha384 = SingleCommand.singleCommand(SHA384.class).parse(args);
		
		if(sha384.helpOption.showHelpIfRequested()) {
			return;
		}
		
		if(AssertUtils.isEmpty(sha384.arguments)) {
			Help.help(sha384.helpOption.commandMetadata);
			return;
		}
		
		sha384.execute(sha384.arguments.toArray(StringUtils.EMPTY_STRING_LIST));
	}
