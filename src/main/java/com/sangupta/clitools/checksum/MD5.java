...
@Command(name = "md5", description = "Compute the MD5 hash of given file/file pattern(s)")
public class MD5 extends AbstractMultiFileTool {
	...
	public static void main(String[] args) {
		MD5 md5 = SingleCommand.singleCommand(MD5.class).parse(args);
		
		if(md5.helpOption.showHelpIfRequested()) {
			return;
		}
		
		if(AssertUtils.isEmpty(md5.arguments)) {
			Help.help(md5.helpOption.commandMetadata);
			return;
		}
		
		md5.execute(md5.arguments.toArray(StringUtils.EMPTY_STRING_LIST));
	}
